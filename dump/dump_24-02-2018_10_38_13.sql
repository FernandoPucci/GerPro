...
CREATE FUNCTION clock_now() RETURNS timestamp without time zone
  ...
  BEGIN
    ...
    IF clock_mocked_at ISNULL OR 
       NOT (select description  = 'DevelopmentDB' from pg_shdescription join pg_database on objoid = pg_database.oid where datname = current_database()) THEN
      RETURN clock_timestamp();
    END IF;
    RAISE NOTICE '============  NECESSARIO MUDAR O COMMENT DO BANCO PARA DevelopmentDB para retornar o clock mockado para o IP %', inet_server_addr()::TEXT;
...
--
-- Name: next_executions_as_json(integer); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION next_executions_as_json(task_check_user_checker_id integer) RETURNS json
    LANGUAGE plpgsql
    AS $$
  DECLARE
    next_executions_json JSON;
  ...
  BEGIN
    ...
    SELECT INTO next_executions_json array_to_json(array_agg(row_to_json(row, false)))
      FROM ( 
        SELECT tc.id, tc.name, tc.description, pl.name AS place_name, tc.starts_at, pm.key_name, tc.repeats_every,
               CASE
                 WHEN pm.key_value = '3' THEN -- Semanal
                   ( SELECT array_to_json(array_agg(row_to_json(row_wd, false)))
                      FROM ( SELECT wd.sunday, wd.monday, wd.tuesday, wd.wednesday, wd.thursday, wd.friday, wd.saturday
                             FROM weeks_days wd WHERE wd.task_check_id = tc.id  
                           ) row_wd 
                   )
                 WHEN pm.key_value = '4' THEN -- Mensal
                   ( SELECT array_to_json(array_agg(row_to_json(row_md, false)))
                       FROM ( SELECT md.month_day FROM months_days md WHERE md.task_check_id = tc.id ORDER BY md.month_day ) row_md 
                   )
                 ELSE  -- Outros
                   '[]'::JSON
               END AS  period,
               ( SELECT array_to_json(array_agg(row_to_json(row_dt, true))) FROM ( SELECT dt.hour, dt.minute FROM days_times dt WHERE dt.task_check_id = tc.id ORDER BY dt.hour, dt.minute ) row_dt ) AS days_times,
               query_task_check_next_execution(tc.id) AS next_execution
          FROM task_checks tc 
            INNER JOIN places pl ON tc.place_id = pl.id
            INNER JOIN parameters pm ON tc.periodicity_id = pm.id
          WHERE tc.user_checker_id = task_check_user_checker_id AND tc.active AND query_task_check_next_execution(tc.id) IS NOT NULL
          ORDER BY next_execution
        ) row;

    RETURN next_executions_json;
  END
$$;


ALTER FUNCTION public.next_executions_as_json(task_check_user_checker_id integer) OWNER TO postgres;

--
-- Name: query_task_check_next_execution(integer); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION query_task_check_next_execution(tc_id integer) RETURNS timestamp without time zone
    LANGUAGE plpgsql
    AS $$
  DECLARE
    task_check_to_query task_checks%ROWTYPE;   -- Usada no loop FOR para inserir na tabela execution_queue
    periodicity INTEGER;                       -- Periodicidade: 1=Unica Vez, 2=Diaria, 3=Semanal, 4=Mensal, 5=Anual

    moment TIMESTAMP;             -- Momento atual + 5 minutos que eh o momento para a proxima execucao
    today DATE;                   -- Data do momento atual
    next_date DATE;	          -- Data da proxima vez da checagem da tarefa
    next_time TIMESTAMP;          -- Data e Hora da proxima vez da checagem da tarefa

    min_time INTEGER;             -- Menor hora + minuto do dia para checagem da tarefa
    max_time INTEGER;             -- Maior hora + minuto do dia para checagem da tarefa

    starts_at_first_week_day DATE;  -- Domingo da semana de starts_at
    next_date_first_week_day DATE;  -- Domingo da semana da proxima verificacao de tarefa
    current_first_week_day DATE;    -- Primeiro dia da semana atual
    weeks_since_first INTEGER;      -- Semanas desde a primeira
    weeks_to_next_date INTEGER;     -- Contagem de semanas ate a proxima checagem de tarefa

    task_check_weeks_days TEXT;  -- Dias da semana nos quais a tarefa deve ser verificada
    week_day INTEGER;            -- Dia da semana (usada para indicar o dia da semana que deverah ser a proxima verificacao da tarefa)

    starts_at_first_month_day DATE;   -- Primeiro dia do mes em que comecou
    current_first_month_day DATE;     -- Primeiro dia do mes atual
    months_since_first INTEGER;       -- Meses desde o primeiro
    months_to_next_date INTEGER;      -- Contagem de meses ate a proxima checagem de tarefa
    next_date_first_month_day DATE;   -- Primeiro dia do mes da proxima execucao
    task_check_month_day INTEGER;     -- Dia da proxima checagem de tarefa

    years_since_first INTEGER;  -- Anos desde que comecou
    years_to_next_date INTEGER; -- Contagem de anos ate a proxima checagem de tarefa
  ...
  BEGIN
    ...
    SELECT INTO task_check_to_query
           tc.*
      FROM task_checks tc
      WHERE tc.id = tc_id;

    SELECT INTO periodicity
           p.key_value::INTEGER AS periodicity_key 
      FROM parameters p 
      WHERE p.id = task_check_to_query.periodicity_id;

    -- EM TODOS OS CASOS ABAIXO LEVA EM CONSIDERACAO repeats_every (repetir a cada dia, semana, mes ou ano)

    IF task_check_to_query.ends_at < today THEN
      -- Ja encerrou, retorna NULL
      RETURN NULL;
    END IF;
    
    CASE periodicity
      WHEN 1 THEN
        -- Checagem da tarefa uma unica vez.
        IF task_check_to_query.starts_at < today THEN
          -- Ja encerrou, retorna NULL
          RETURN NULL;
        END IF;
        next_date := task_check_to_query.starts_at;  -- Proxima checagem eh as mesma data de inicio da checagem da tarefa
      WHEN 2 THEN
        -- Checagem da tarefa for diaria.
        next_date := task_check_to_query.starts_at + 
                     (
                        task_check_to_query.repeats_every * 
                        ceil( (clock_now()::DATE - task_check_to_query.starts_at)::FLOAT / task_check_to_query.repeats_every )::INTEGER 
                     );
    
      WHEN 3 THEN        
        -- Checagem da tarefa for Semanal, considera o se o dia da semana de next_date estah nos dias da semana marcados para fazer a checagem da tarefa.
        -- Se nao estiver, considera o proximo dia da semana subsequente a next_date, levando em consideracao que poderah ser na proxima semana.
        starts_at_first_week_day := task_check_to_query.starts_at - 
                                    extract('dow' FROM task_check_to_query.starts_at)::INTEGER;    -- Primeiro dia da semana em que comecou
        current_first_week_day := clock_now()::DATE - extract('dow' FROM clock_now())::INTEGER;    -- Primeiro dia da semana atual
        weeks_since_first :=  (current_first_week_day - starts_at_first_week_day)::FLOAT / 7;      -- Semanas desde a primeira
        weeks_to_next_date := ceil( ( (current_first_week_day - starts_at_first_week_day)::FLOAT /
                                      (7 * task_check_to_query.repeats_every)::FLOAT
                                    )
                              )::INTEGER * task_check_to_query.repeats_every;                                                                   -- Contagem de semanas ate a proxima checagem de tarefa

        next_date_first_week_day := starts_at_first_week_day + weeks_to_next_date * 7;   -- Primeiro dia da semana da proxima execucao

        -- Dias da semana nos quais a tarefa deve ser verificada (1 para SIM e 0 para NAO).
        SELECT INTO task_check_weeks_days
          '' || sunday::INTEGER || monday::INTEGER || tuesday::INTEGER || wednesday::INTEGER || thursday::INTEGER || friday::INTEGER || saturday::INTEGER
          FROM weeks_days 
          WHERE task_check_id = tc_id;

        week_day := extract('dow' FROM clock_now()) + 1;
        WHILE week_day < 7  LOOP
          IF substring(task_check_weeks_days FROM week_day FOR 1) = '1' THEN
            EXIT;
          END IF;
          week_day := week_day + 1;
        END LOOP;

        IF substring(task_check_weeks_days FROM week_day FOR 1) != '1' THEN  -- Procurou ateh o sabado e nao encontrou nada
          next_date_first_week_day := next_date_first_week_day + task_check_to_query.repeats_every * 7;   -- Primeiro dia da semana da proxima execucao ajustado para mais um periodo
          week_day := 1;  -- Volta o dia para o domingo (primeiro dia da semana semana)
          -- Passou do sabado entao comeca do domingo ateh encontrar o proximo dia
          WHILE week_day < extract('dow' FROM clock_now()) + 1  LOOP
            IF substring(task_check_weeks_days FROM week_day FOR 1) = '1' THEN
              EXIT;
            END IF;
            week_day := week_day + 1;
        ...
        IF substring(task_check_weeks_days FROM week_day FOR 1) != '1' THEN  -- Procurou ateh o sabado e nao encontrou nada entao retornarah NULL
          RETURN NULL;
        END IF;

        next_date := next_date_first_week_day + week_day - 1;

      WHEN 4 THEN
        -- Se a checagem da tarefa for Mensal, considera o se o dia do mes de next_date estah nos dias do mes marcados para fazer a checagem da tarefa.
        -- Se nao estiver, considera o proximo dia do mes subsequente a next_date, levando em consideracao que poderah ser no proximo mes.
        starts_at_first_month_day := task_check_to_query.starts_at - 
                                    extract('day' FROM task_check_to_query.starts_at)::INTEGER + 1;    -- Primeiro dia do mes em que comecou
        current_first_month_day := clock_now()::DATE - extract('day' FROM clock_now())::INTEGER + 1;    -- Primeiro dia do mes atual
        months_since_first :=  extract('years' FROM age(current_first_month_day, starts_at_first_month_day))::INTEGER * 12 + 
                               extract('months' FROM age(current_first_month_day, starts_at_first_month_day))::INTEGER;     -- Meses desde o primeiro

        months_to_next_date := ceil( ( months_since_first::FLOAT /
                                      (task_check_to_query.repeats_every)::FLOAT
                                    )
                              )::INTEGER * task_check_to_query.repeats_every;                                                                   -- Contagem de meses ate a proxima checagem de tarefa

        next_date_first_month_day := starts_at_first_month_day + ( months_to_next_date * interval '1 month' );   -- Primeiro dia do mes da proxima execucao

        -- Dia do mes no qual a tarefa deve ser verificada
        SELECT INTO task_check_month_day month_day
          FROM months_days
          WHERE task_check_id = tc_id AND
                month_day >= extract('day' FROM CASE WHEN next_date_first_month_day < today THEN today ELSE next_date_first_month_day END)::INTEGER
          ORDER BY 1
          LIMIT 1;
        IF task_check_month_day ISNULL THEN
          SELECT INTO task_check_month_day month_day
            FROM months_days
            WHERE task_check_id = tc_id
            ORDER BY 1
            LIMIT 1;
          next_date_first_month_day := next_date_first_month_day + 1 * interval '1 month';
        END IF;
        
        next_date := next_date_first_month_day + task_check_month_day - 1;
        
      WHEN 5 THEN
        -- Se a checagem da tarefa for Anual, considera o se o dia e o mes do ano de next_date eh igual ao dia e mes do ano para fazer a checagem da tarefa.
        -- Se nao estiver, considera o proximo dia e mes subsequente a next_date, levando em consideracao que poderah ser no proximo ano.
        IF task_check_to_query.starts_at > clock_now()::DATE THEN
          next_date := task_check_to_query.starts_at;
        ELSE
          years_since_first :=  extract('years' FROM age(clock_now()::DATE, task_check_to_query.starts_at))::INTEGER;     -- Anos desde que comecou
          years_to_next_date := ceil( ( years_since_first::FLOAT /
                                      (task_check_to_query.repeats_every)::FLOAT
                                    )
                              )::INTEGER * task_check_to_query.repeats_every + 1;   -- Contagem de anos ate a proxima checagem de tarefa
          next_date := task_check_to_query.starts_at + years_to_next_date * interval '1 year';
        END IF;
        RAISE NOTICE '------------------------------- task_check_to_query.starts_at = %     years_since_first = %     years_to_next_date = %     next_date = %',
                     task_check_to_query.starts_at, years_since_first, years_to_next_date, next_date;
    END CASE;

    -- Seleciona a hora da proxima execucao
    -- Momento de iniciar a notificacao: Dia atual + Se(passou da hora? 1 dia, 0 dia) + hora e minuto da verificacao da tarefa

    -- Menor e maior hora e minutos do dia para checagem da tarefa
    SELECT INTO min_time, max_time min(dt.hour * 60 + dt.minute) AS min_time, max(dt.hour * 60 + dt.minute) AS max_time
      FROM days_times dt
      WHERE dt.task_check_id = tc_id;
    IF next_date = today THEN
      -- O dia da proxima checagem de tarefa eh hoje
      IF moment > today + min_time * (interval '1 minute') THEN
        -- Ja passou da menor hora + minuto do dia
        IF moment > today + max_time * (interval '1 minute') THEN
          -- Ja passou da maior hora + minuto do dia entao considera o proximo dia + a menor (hora + minuto)
          next_time := next_date + 
                       task_check_to_query.repeats_every *
                       ( CASE periodicity
                           WHEN 1 THEN  -- Unica vez
                             NULL
                           WHEN 2 THEN  -- Diario
                             1 * (interval '1 day')
                           WHEN 3 THEN  -- Semanal
                       ...
                       min_time * (interval '1 minute');
        ELSE
          -- Nao passou da maior hora + minuto do dia entao considera o a menor (hora + minuto)
          next_time := next_date + max_time * interval '1 minute';
        END IF;
      ELSE
        next_time := next_date + min_time * interval '1 minute';
      END IF;
    ELSE 
      next_time := next_date + min_time * interval '1 minute';
    END IF;

    RETURN next_time;  -- Data e hora da proxima vez da checagem de tarefa
  END
$$;


ALTER FUNCTION public.query_task_check_next_execution(tc_id integer) OWNER TO postgres;
...
CREATE FUNCTION next_executions_as_json_test_001() RETURNS TABLE(function_name character varying, test_passed boolean, details text, running_time interval)
    LANGUAGE plpgsql
    AS $$
  DECLARE
    start_time_test TIMESTAMP;
    end_time_test TIMESTAMP;
    returned BOOLEAN;
  BEGIN
    start_time_test := clock_now();

    returned := (SELECT (COUNT(*) > 0) AS elements FROM json_array_elements(next_executions_as_json(2)));
    ...
    (
      SELECT t.function_name, t.test_passed, t.details, t.running_time 
        FROM ( SELECT 'next_executions_as_json(checker_user_id BOOLEAN)'::VARCHAR(64) AS function_name,
                      (returned)
                      AS test_passed,
                      ( E'Teste 001\n'||
                        E'SELECT COUNT(*) > 0 from json_array_elements(next_executions_as_json(2))\n' ||
                        'Esperados: true' || E'\n' || 
                        'Retornados: ' || coalesce(returned || '', '<NULL>' ))::TEXT
                      AS details,
                      end_time_test - start_time_test AS running_time
             ) t
    );
...
ALTER FUNCTION tests.next_executions_as_json_test_001() OWNER TO postgres;

--
-- Name: query_task_check_next_execution_test_001(); Type: FUNCTION; Schema: tests; Owner: postgres
--

CREATE FUNCTION query_task_check_next_execution_test_001() RETURNS TABLE(function_name character varying, test_passed boolean, details text, running_time interval)
    LANGUAGE plpgsql
    AS $$
  DECLARE
    place_id_test INTEGER;                -- id do local para usar nos testes
    periodicity_id_test INTEGER;          -- id da periodicidade usada nos testes
    start_date_test DATE;                 -- Data para o teste
    days_time_hour_to_test INTEGER[1];    -- Hora para o Teste
    days_time_minute_to_test INTEGER[1];  -- Minuto para o Teste
    task_check_id_test INTEGER;           -- array com os id da verificacao de tarefa que serah testada
    tc_id INTEGER;                        -- id que será armazenado em task_check_id_test

    start_time_test TIMESTAMP;            -- Momento de inicio do teste
    end_time_test   TIMESTAMP;            -- Momento de fim do teste
    moment_to_test  TEXT;                 -- Momento em que eh ajustado o mock do relogio para executar o teste (simula o momento atual)

    task_check_test TEXT;                 -- String JSON com a tarefa a ser inserida e testada

    task_check_name TEXT;                 -- Nome da checagem de tarefa testada
    task_check_description TEXT;          -- Descricao da checagem de tarefa testada
    next_execution_expected TIMESTAMP;    -- Momentos esperados das proximas execucoes
    next_execution TIMESTAMP;             -- Momentos das proximas execucoes

  BEGIN
    days_time_hour_to_test := array[ NULL, NULL, NULL ];
    days_time_minute_to_test := array[ NULL, NULL, NULL ];
  
    -- Local que serah usado para todos os testes
    INSERT INTO places (company_id, name, updated_by_user_id) VALUES (1, 'Local apenas para testes das funcoes do banco de dados', 1) RETURNING id INTO place_id_test;

    BEGIN
      --------------------------- Teste 1 - Unica vez e jah passou da data da checagem ---------------------------
      PERFORM tests.set_clock_now('2018-01-02T09:52:00');  -- Faz mock do relogio
      start_time_test := clock_now();

      SELECT INTO periodicity_id_test id FROM parameters WHERE category_name = 'periodicidades' AND key_name = 'unica_vez';

      start_date_test := clock_now()::DATE - interval '1 days';

      task_check_name = '"Teste 1 - Unica vez e jah passou da data da checagem"';
      task_check_description = '"Retornarah NULL para a proxima checagem"';
      days_time_hour_to_test[1] := 10;
      days_time_minute_to_test[1] := 0;
      next_execution_expected := NULL;

      task_check_test :=
        '{ "name": ' || task_check_name || ',
           "description": ' || task_check_description || ',
           "place_id": ' || place_id_test || ',
           "user_checker_id": 2,
           "periodicity_id": ' || periodicity_id_test || ',
           "starts_at": "' || start_date_test || '",
           "ends_at": null,
           "repeats_every": 1,
           "updated_by_user_id": 1,
           "days_times": [ { "hour": ' || days_time_hour_to_test[1] || ', "minute": ' || days_time_minute_to_test[1] || ' } ],
      ...
      SELECT INTO tc_id insert_taskcheckjson(task_check_test);
      task_check_id_test := tc_id;

      PERFORM tests.set_clock_now('2018-01-04T09:52:00');  -- Faz mock do relogio para passar da data de checagem e retornar NULL

      -- Pega a proxima execucao e guarda na next_execution
      next_execution = query_task_check_next_execution(task_check_id_test);
    END;

    BEGIN
      ---------------------- Exclui tuplas criadas para testes -----------------------
      DELETE FROM execution_queue WHERE task_check_id = task_check_id_test;
      DELETE FROM task_checks WHERE id = task_check_id_test;
      DELETE FROM places WHERE id = place_id_test;
    END;

    end_time_test := clock_now();

    PERFORM tests.set_clock_now(NULL);  -- Desfaz mock do relogio

    RETURN QUERY
    (
      SELECT t.function_name, t.test_passed, t.details, t.running_time 
        FROM ( SELECT 'query_task_check_next_execution()'::VARCHAR(64) AS function_name,
                      (coalesce(date_trunc('second',next_execution) || '', '<NULL>') = coalesce(date_trunc('second', next_execution_expected) || '', '<NULL>'))
                      AS test_passed,
                      ( task_check_name || E'\n' || task_check_description || E'\n' || 
                        'Esperado: next_execution_expected = ' || coalesce(date_trunc('second', next_execution_expected) || '', '<NULL>') || E'\n' || 
                        'Retornado: next_execution = ' || coalesce(date_trunc('second',next_execution) || '', '<NULL>') )::TEXT
                      AS details,
                      end_time_test - start_time_test AS running_time
             ) t
    );
...
ALTER FUNCTION tests.query_task_check_next_execution_test_001() OWNER TO postgres;

--
-- Name: query_task_check_next_execution_test_002(); Type: FUNCTION; Schema: tests; Owner: postgres
--

CREATE FUNCTION query_task_check_next_execution_test_002() RETURNS TABLE(function_name character varying, test_passed boolean, details text, running_time interval)
    LANGUAGE plpgsql
    AS $$
  DECLARE
    place_id_test INTEGER;                -- id do local para usar nos testes
    periodicity_id_test INTEGER;          -- id da periodicidade usada nos testes
    start_date_test DATE;                 -- Data para o teste
    days_time_hour_to_test INTEGER[];     -- Hora para o Teste
    days_time_minute_to_test INTEGER[];   -- Minuto para o Teste
    task_check_id_test INTEGER;           -- array com os id da verificacao de tarefa que serah testada
    tc_id INTEGER;                        -- id que será armazenado em task_check_id_test
  ...
  BEGIN
    ...
    BEGIN
      --------------------------- Teste 2 - Diaria e comeca hoje as 10:00:00 ---------------------------
      PERFORM tests.set_clock_now('2018-01-13T09:37:00');  -- Faz mock do relogio
      start_time_test := clock_now();

      SELECT INTO periodicity_id_test id FROM parameters WHERE category_name = 'periodicidades' AND key_name = 'diario';

      start_date_test := clock_now()::DATE;

      task_check_name = '"Teste 2 - Diaria e comeca hoje as 10:00:00"';
      task_check_description = '"Retornarah 2018-01-13T10:00:00 para a proxima checagem"';
      days_time_hour_to_test[1] := 10;
      days_time_minute_to_test[1] := 0;
      next_execution_expected := clock_now()::DATE + interval '10 hours';
      ...
      -- Pega a proxima execucao e guarda na next_execution[1]
      next_execution = query_task_check_next_execution(task_check_id_test);
    END;
      ...
      SELECT t.function_name, t.test_passed, t.details, t.running_time 
        FROM ( SELECT 'query_task_check_next_execution()'::VARCHAR(64) AS function_name,
                      ...
                      ( task_check_name || E'\n' || task_check_description || E'\n' || 
                        ...
                        'Retornados: next_execution = ' || coalesce(date_trunc('second',next_execution) || '', '<NULL>') )::TEXT
                      AS details,
                      end_time_test - start_time_test AS running_time
             ) t
    );
...
ALTER FUNCTION tests.query_task_check_next_execution_test_002() OWNER TO postgres;

--
-- Name: query_task_check_next_execution_test_003(); Type: FUNCTION; Schema: tests; Owner: postgres
--

CREATE FUNCTION query_task_check_next_execution_test_003() RETURNS TABLE(function_name character varying, test_passed boolean, details text, running_time interval)
    LANGUAGE plpgsql
    AS $$
  DECLARE
    place_id_test INTEGER;                -- id do local para usar nos testes
  ...
  BEGIN
    ...
    BEGIN
      --------------------------- Teste 3 - Semanal a cada 2 sabados e comecou sabado passado, nos horarios das 7:30:00 ou 13:40:00 ---------------------------
      PERFORM tests.set_clock_now('2018-01-13T10:22:00');  -- Faz mock do relogio
      start_time_test := clock_now();

      SELECT INTO periodicity_id_test id FROM parameters WHERE category_name = 'periodicidades' AND key_name = 'semanal';

      start_date_test := clock_now()::DATE;
      moment_to_test := '2018-01-27T14:29:00';

      task_check_name = '"Teste 3 - Semanal a cada 2 sabados"';
      task_check_description = '"comecou sabado passado, ' || start_date_test || ' nos horarios das 7:30:00 ou 13:40:00 e o momento atual eh ' || moment_to_test || '"';
      days_time_hour_to_test[1] := 7;
      days_time_minute_to_test[1] := 30;
      days_time_hour_to_test[2] := 13;
      days_time_minute_to_test[2] := 40;

      next_execution_expected := '2018-02-10T07:30:00';
      ...
      task_check_test :=
        '{ "name": ' || task_check_name || ',
           ...
           "ends_at": null,
           "repeats_every": 2,
           "weeks_days": { "sunday": false, "monday": false, "tuesday": false, "wednesday": false, "thursday": false, "friday": false, "saturday": true },
           "updated_by_user_id": 1,
           "days_times": [ { "hour": ' || days_time_hour_to_test[1] || ', "minute": ' || days_time_minute_to_test[1] || ' },
                           { "hour": ' || days_time_hour_to_test[2] || ', "minute": ' || days_time_minute_to_test[2] || ' }
           ],
           "notifications": [ { "notification_type_id": 19, "pre_notify_days": 0, "pre_notify_hours": 0, "pre_notify_minutes": 0, "notify_again_every": 0 } ]
         }';
      ...
      -- Pega a proxima execucao e guarda na next_executions[1]
      PERFORM tests.set_clock_now(moment_to_test);  -- Faz mock do relogio para 12 dias depois que a tarefa foi cadastrada
      next_execution := query_task_check_next_execution(task_check_id_test);
    END;
...
ALTER FUNCTION tests.query_task_check_next_execution_test_003() OWNER TO postgres;

--
-- Name: query_task_check_next_execution_test_004(); Type: FUNCTION; Schema: tests; Owner: postgres
--

CREATE FUNCTION query_task_check_next_execution_test_004() RETURNS TABLE(function_name character varying, test_passed boolean, details text, running_time interval)
    LANGUAGE plpgsql
    AS $$
  DECLARE
    place_id_test INTEGER;                -- id do local para usar nos testes
  ...
  BEGIN
    days_time_hour_to_test := array[ NULL ];
    days_time_minute_to_test := array[ NULL ];
  
    -- Local que serah usado para todos os testes
    INSERT INTO places (company_id, name, updated_by_user_id) VALUES (1, 'Local apenas para testes das funcoes do banco de dados', 1) RETURNING id INTO place_id_test;

    BEGIN
      --------------------------- Teste 4 - Mensal a cada 2 meses e comecou 2018-01-13 passado, no horario das 9:30:00  ---------------------------
      PERFORM tests.set_clock_now('2018-01-13T10:22:00');  -- Faz mock do relogio
      start_time_test := clock_now();
      ...
      task_check_name = '"Teste 4 - Mensal todos dias 5 e 20"';
      task_check_description = '"comecou sabado passado, ' || start_date_test || '. Todos dias 5 e 20 no horario das 9:30 e o momento atual eh ' || moment_to_test || '"';
      days_time_hour_to_test[1] := 9;
      days_time_minute_to_test[1] := 30;

      next_execution_expected := '2018-02-05T09:30:00';
      ...
      task_check_test :=
        '{ "name": ' || task_check_name || ',
           ...
           "ends_at": null,
           "repeats_every": 2,
           "months_days": [ {"month_day": 5}, {"month_day": 20} ],
           "updated_by_user_id": 1,
           "days_times": [ { "hour": ' || days_time_hour_to_test[1] || ', "minute": ' || days_time_minute_to_test[1] || ' } ],
           "notifications": [ { "notification_type_id": 19, "pre_notify_days": 0, "pre_notify_hours": 0, "pre_notify_minutes": 0, "notify_again_every": 0 } ]
         }';
...
ALTER FUNCTION tests.query_task_check_next_execution_test_004() OWNER TO postgres;

--
-- Name: query_task_check_next_execution_test_005(); Type: FUNCTION; Schema: tests; Owner: postgres
--

CREATE FUNCTION query_task_check_next_execution_test_005() RETURNS TABLE(function_name character varying, test_passed boolean, details text, running_time interval)
    LANGUAGE plpgsql
    AS $$
  DECLARE
    place_id_test INTEGER;                -- id do local para usar nos testes
  ...
  BEGIN
    ...
    BEGIN
      --------------------------- Teste 005 - Anual a cada 1 ano e comecou 2018-02-02, no horario das 9:30:00  ---------------------------
      PERFORM tests.set_clock_now('2018-01-13T10:22:00');  -- Faz mock do relogio
      start_time_test := clock_now();

      SELECT INTO periodicity_id_test id FROM parameters WHERE category_name = 'periodicidades' AND key_name = 'anual';

      start_date_test := '2018-02-02'::DATE;
      moment_to_test := '2018-01-27T14:29:00';

      task_check_name = '"Teste 005 - Anual todo dia 2 de Fevereiro"';
      task_check_description = '"comecou em ' || start_date_test || '. Todos dias 2 de Fevereiro no horario das 9:30 e o momento atual eh ' || moment_to_test || '"';
      days_time_hour_to_test[1] := 9;
      days_time_minute_to_test[1] := 30;

      next_execution_expected := '2018-02-02T09:30:00';
...
ALTER FUNCTION tests.query_task_check_next_execution_test_005() OWNER TO postgres;

--
-- Name: query_task_check_next_execution_test_006(); Type: FUNCTION; Schema: tests; Owner: postgres
--

CREATE FUNCTION query_task_check_next_execution_test_006() RETURNS TABLE(function_name character varying, test_passed boolean, details text, running_time interval)
    LANGUAGE plpgsql
    AS $$
  DECLARE
    place_id_test INTEGER;                -- id do local para usar nos testes
  ...
  BEGIN
    ...
    BEGIN
      --------------------------- Teste 6 - Anual a cada 1 ano e comecou 2017-07-29, no horario das 12:00:00  ---------------------------
      PERFORM tests.set_clock_now('2017-01-13T10:22:00');  -- Faz mock do relogio
      start_time_test := clock_now();

      SELECT INTO periodicity_id_test id FROM parameters WHERE category_name = 'periodicidades' AND key_name = 'anual';

      start_date_test := '2017-07-29'::DATE;
      moment_to_test := '2018-02-13T11:23:00';

      task_check_name = '"Teste 006 - Anual todo dia 29 de Julho"';
      task_check_description = '"comecou em ' || start_date_test || '. Todos dias 29 de Julho no horario das 12:00 e o momento atual eh ' || moment_to_test || '"';
      days_time_hour_to_test[1] := 12;
      days_time_minute_to_test[1] := 0;

      next_execution_expected := '2018-07-29T12:00:00';
...
ALTER FUNCTION tests.query_task_check_next_execution_test_006() OWNER TO postgres;

--
-- Name: set_clock_now(text); Type: FUNCTION; Schema: tests; Owner: postgres
--
...
CREATE FUNCTION set_clock_now(time_to_clock text) RETURNS void
  ...
  BEGIN
    -- Para usar essa funcao eh necessario criar um parametro que indica que o banco de dados eh o de desenvolvimento com a instrucao abaixo:
    ...
    IF ( SELECT count(id) FROM parameters WHERE category_name = 'Tests' AND key_name = 'DevelopmentDB' AND key_value = 'true' ) = 0 THEN
      RAISE EXCEPTION E'\n======================================================
...
ALTER FUNCTION tests.set_clock_now(time_to_clock text) OWNER TO postgres;

--
-- Name: test_all(boolean, text); Type: FUNCTION; Schema: tests; Owner: postgres
--

CREATE FUNCTION test_all(only_fails boolean, also_function_name text) RETURNS TABLE(function_name character varying, test_passed boolean, details text, running_time interval)
    LANGUAGE plpgsql
    AS $$
  BEGIN
    ...
    ( 
      ( SELECT t.function_name, t.test_passed, t.details, t.running_time 
        FROM ( SELECT * FROM tests.query_task_check_next_execution_test_001()
               UNION SELECT * FROM tests.query_task_check_next_execution_test_002()
               UNION SELECT * FROM tests.query_task_check_next_execution_test_003()
               UNION SELECT * FROM tests.query_task_check_next_execution_test_004()
               UNION SELECT * FROM tests.query_task_check_next_execution_test_005()
               UNION SELECT * FROM tests.query_task_check_next_execution_test_006()
               UNION SELECT * FROM tests.next_executions_as_json_test_001()
             ) t
        WHERE ( NOT coalesce(only_fails, false) OR NOT t.test_passed ) OR
              ( CASE WHEN coalesce(also_function_name, '') = '' THEN NULL ELSE t.function_name LIKE '%' || also_function_name ||'%' END )
        ORDER BY function_name, details
      )
    );
  END
$$;
...
ALTER FUNCTION tests.test_all(only_fails boolean, also_function_name text) OWNER TO postgres;
...
00037_replace_function_clock_now	2018-02-24 10:25:31.225345
00038_create_schema_tests	2018-02-24 10:25:37.338917
00039_move_set_clock_now_to_tests_schema	2018-02-24 10:25:40.751474
00040_create_function_query_task_check_next_execution	2018-02-24 10:25:44.441314
00041_create_function_tests.query_task_check_next_execution_test_001	2018-02-24 10:25:48.901986
00042_create_function_tests.query_task_check_next_execution_test_002	2018-02-24 10:25:52.814062
00043_create_function_tests.query_task_check_next_execution_test_003	2018-02-24 10:25:57.425501
00044_create_function_tests.query_task_check_next_execution_test_004	2018-02-24 10:26:01.44406
00045_create_function_tests.query_task_check_next_execution_test_005	2018-02-24 10:26:05.02758
00046_create_funcion_tests.test_all	2018-02-24 10:26:08.495432
00047_create_funcion_next_executions_as_json	2018-02-24 10:26:12.304457
00048_create_function_tests.query_task_check_next_execution_test_006	2018-02-24 10:26:15.510111
00049_replace_function_query_task_check_next_execution	2018-02-24 10:26:18.244681
00050_create_function_next_executions_as_json_test_001	2018-02-24 10:26:21.248469
00051_replace_funcion_tests.test_all	2018-02-24 10:26:25.044464
\.
